...
@section('title', 'Tambah Kosa Kata')

@section('content')
<div class="container mx-auto p-4 font-nunito">
    <div class="bg-white shadow-lg rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Tambah Kosa Kata</h2>
            <p class="text-sm text-gray-600 mt-1">Buat kosa kata baru.</p>
        </div>

        <div class="p-6">
            <form action="{{ route('dashboard.vocabularies.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="space-y-6">
                    <div>
                        ...
                        <select name="vocabulary_category_id" id="vocabulary_category_id"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ old('vocabulary_category_id') == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="example" id="example" rows="4"
                                  ...
                                  placeholder="Masukkan contoh penggunaan (opsional)">{{ old('example') }}</textarea>
                        @error('example')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
