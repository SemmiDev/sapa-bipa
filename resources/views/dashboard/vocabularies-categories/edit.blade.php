...
@section('title', 'Edit Kategori Kosa Kata')

@section('content')
<div class="container mx-auto p-4 font-nunito">
    <div class="bg-white shadow-lg rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Edit Kategori Kosa Kata</h2>
            <p class="text-sm text-gray-600 mt-1">Perbarui informasi kategori kosa kata.</p>
        </div>

        <div class="p-6">
            <form action="{{ route('dashboard.vocabularies-categories.update', $vocabularyCategory) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700">Nama Kategori</label>
                        <input type="text" name="name" id="name" value="{{ old('name', $vocabularyCategory->name) }}"
                    ...
                    <div>
                        ...
                        @if ($vocabularyCategory->thumbnail_url)
                            <img src="{{ $vocabularyCategory->thumbnail_url }}" alt="{{ $vocabularyCategory->name }}"
