    ...
    <div class="bg-white shadow-lg rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Kelola artikel Anda.</p>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($articles as $article)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $article->articleCategory->name }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    ...
                                    <button type="button" data-modal-target="delete-modal-{{ $article->id }}"
                            ...
                            <!-- Modal Hapus -->
                            <div id="delete-modal-{{ $article->id }}" tabindex="-1"
                                 class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow">
                                        <div class="p-4 text-center">
                                            <h3 class="mb-5 text-lg font-semibold text-gray-800">Hapus Artikel</h3>
                                            <p class="mb-5 text-sm text-gray-600">Apakah Anda yakin ingin menghapus artikel "{{ $article->title }}"?</p>
                                            <form action="{{ route('dashboard.articles.destroy', $article) }}" method="POST">
                                                ...
                                                <button type="button" data-modal-hide="delete-modal-{{ $article->id }}"
